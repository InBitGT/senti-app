...
export const ModalMovementDetail: React.FC<Props> = ({
  isOpen,
  onClose,
  data,
}) => {
      ...
      <ModalContent style={styles.container}>
        ...
        <ModalBody>
          ...
          <View style={styles.section}>
            ...
            <InfoRow
              label="Por defecto"
              value={data?.warehouse?.is_default ? "Sí" : "No"}
            />
          </View>
          ...
          {data?.batch && (
            <>
              ...
              <View style={styles.section}>
                ...
                <InfoRow
                  label="Costo unit."
                  value={`Q${data.batch.unit_cost.toFixed(2)}`}
                />
                <InfoRow
                  label="Vencimiento"
                  value={
                    data.batch.expiration_date
                      ? new Date(data.batch.expiration_date).toLocaleDateString(
                          "es-GT",
                        )
                      : undefined
                  }
